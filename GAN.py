...
for epoch in tqdm(range(epochs)):
    for batch_idx, (images_real, _) in enumerate(loader):
        ...
        loss_disc_real = loss(disc_images_real, torch.ones_like(disc_images_real)) # log(D(x))

        disc_images_fake = discriminator(images_fake).view(-1) # D(G(z))
        loss_disc_fake = loss(disc_images_fake, torch.zeros_like(disc_images_real)) # log(1 - D(G(z)))
        ...
        loss_gen = loss(disc_images_fake2, torch.ones_like(disc_images_fake))
        optim_gen.zero_grad()
        loss_gen.backward()
        optim_gen.step()
